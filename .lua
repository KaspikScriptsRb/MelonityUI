-- mui_library.lua
-- Modern UI library inspired by the screenshot layout
...
function MUILib:CreateWindow(opts)
    ...
    dropIcon.Image = "rbxassetid://6031090990" -- down chevron
    ...
    local function createLangOption(text, order)
        ...
        btn.MouseEnter:Connect(function()
            tween(btn, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextColor3 = defaultTheme.TextPrimary})
        end)
        btn.MouseLeave:Connect(function()
            tween(btn, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextColor3 = defaultTheme.TextSecondary})
        end)
        return btn
    end
    ...
    langButton.MouseButton1Click:Connect(function()
        ...
        local target = langMenu.Visible and 1 or 0
        langMenu.ClipsDescendants = true
        langMenu.Size = UDim2.new(0, 110, 0, 0)
        tween(langMenu, TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.new(0, 110, 0, 56)})
        tween(dropIcon, TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Rotation = langMenu.Visible and 180 or 0})
    end)
    ...
    local tabsPadding = Instance.new("UIPadding", topTabs)
    ...
    navListLayout.Parent = navListHolder
    ...
    -----------------------------------------------------------------
    -- RIGHT PANELS CONTAINER (for sections etc.)
    -----------------------------------------------------------------
    ...
    function tab:CreateSection(title)
        ...
        function section:AddToggle(opts)
            ...
            local function set(v)
                state = v
                if state then
                    tween(button, TweenInfo.new(0.14, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundColor3 = defaultTheme.Accent})
                    tween(dot, TweenInfo.new(0.14, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Position = UDim2.new(1, -18, 0.5, -8)})
                else
                    tween(button, TweenInfo.new(0.14, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundColor3 = Color3.fromRGB(60, 60, 60)})
                    tween(dot, TweenInfo.new(0.14, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Position = UDim2.new(0, 2, 0.5, -8)})
                end
                callback(state)
            end
